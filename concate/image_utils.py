...
def plot_confusion_matrix(cm, labels_name, title, acc, num_classes):
    ...
    plt.imshow(cm, interpolation='none')  # 在特定的窗口上显示图像nearest
    plt.title(title)  # 图像标题
    plt.colorbar()
    num_class = np.array(range(len(labels_name)))  # 获取标签的间隔数
    plt.xticks(num_class, labels_name, rotation=45, fontsize=16)  # 将标签印在x轴坐标上
    plt.yticks(num_class, labels_name, fontsize=16)  # 将标签印在y轴坐标上
    plt.ylabel('Target')
