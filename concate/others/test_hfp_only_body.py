...
def parse_args():
    ...
    parser.add_argument('--checkpoint_face', type=str, default=None, help='Pytorch checkpoint file path')
...
def test():
    ...
    if args.phase != 'new_test':
        ...
        feature_map_face =[]
        storage_face = np.zeros((110,7))
        storage_body = np.zeros((110,7))
        video_img_count = 0
        # whole_predicts = []
        ...
        with torch.no_grad():
            ...
            '''Body Output'''
            data =get_babyrobot_data(phase = 'test')
            bodies, hands_right, hands_left, lengths, label, paths =data
            video_img_count = 0
            for batch_i, (batch) in enumerate(body_test_loader):
                body, hand_right, hand_left, length, Y = batch['body'].to(device),\
                                                     batch['hand_right'].to(device),\
                                                     batch['hand_left'].to(device),\
